...
-- student 테이블의 st_num 칼럼에 모두 있기 때문에 EQ JOIN을 실행해서
-- 데이터를 확인 할 수 있다.
SELECT *
...
-- SELECT * 는 데이터가 많을 경우 조회하는데 다소 시간이 많이 소요되기도 하다
...
/*
값과 값을 연결하여 하나의 문자열처럼 출력하기. 각각의 DBMS마다 조금씩 문법이 다르다
...
SELECT * FROM view_score WHERE d_name
LIKE '컴퓨터' || '%'; -- JAVA 등 프로그래밍 코딩으로 SQL을 작성할 때는 LIKE 키워드의 문자열을 연결문자열로 작성해야한다

SELECT DISTINCT s_dept, d_name FROM view_score ORDER BY s_dept; -- 데이터가 다수 존재할 때 중복되지 않는 데이터만 출력하는 키워드
-- 같은 컬럼에 있는 동일한 값은 한번만 출력

SELECT DISTINCT s_dept, d_name, s_num FROM view_score ORDER BY s_dept; -- 다수의 컬럼을 DISTINCT하면 원하는 결과가 나오지 않는다

SELECT s_dept, d_name FROM view_score GROUP BY s_dept, d_name ORDER BY s_dept; -- 표준 SQL로 DISTINCT와 같은 결과를 보려면 GROUP BY
-- GROUP BY. 특정 컬럼을 기준으로 하여 집계를 할 때 사용하는 명령절

-- 학과별로 국어점수의 총점을 계산하고 학과 번호 순으로
SELECT s_dept, d_name, d_pro, SUM(s_kor) FROM view_score GROUP BY s_dept, d_name, d_pro ORDER BY s_dept; -- 부분합 집계
-- GROUP BY로 묶어서 부분합을 보고자 할 때 기준으로 하는 컬럼 이외에 SELECT 문에 나열된 컬럼들 중 집계함수로 감싸지 않은 컬럼들은 GROUP BY 다음에 나열을 해주어야한다

SELECT s_num, s_dept, d_name, d_pro, SUM(s_kor) FROM view_score GROUP BY s_num, s_dept, d_name, d_pro ORDER BY s_dept;
-- 학번, 학과코드로 묶어서 보여주는 데이터는 아무 의미가 없다. GROUP으로 묶어서 집계를 낼 때는 어떤 컬럼들을 묶을 것인지에 대한 많은 고민을 해야한다


-- 컴퓨터공학, 영어영문 학과만 추출하여 집계
SELECT s_dept, d_name, d_pro, SUM(s_kor) AS 국어총점, SUM(s_eng) AS 영어총점, SUM(s_math) AS 수학총점
FROM view_score
WHERE d_name IN ('컴퓨터공학', '영어영문')
GROUP BY s_dept, d_name, d_pro
ORDER BY s_dept;
-- GROUP BY를 실행할 때 조건을 부여하는 방법으로 WHERE와 HAVING, 2가지가 있다