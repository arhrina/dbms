...
/*
iolist를 제2정규화를 수행해서 상품정봐 거래처정보를 TABLE 분리완성
...
iolist의 매입, 매출단가는 실제로 상품이 매입 매출되는 시점에 변동될수 있다.
    ...
    단가가 달리 적용된다.
    기준단가(100개) : 1000, 권장수량(1000개) : 900, 밀어내기단기(5000개) 700 
...
-- 5. 3자리마다 , 찍어 보이기
--      999,999 : 출력포맷 형식, 실제 표시되는 값보다 충문히 큰 자릴수 지정
...
SELECT SUBSTR(IODATE,0,7) AS 월,
    TO_CHAR(SUM(DECODE(INOUT,1,AMT)),'999,999,999') AS 매입합계,
    TO_CHAR(SUM(DECODE(INOUT,2,AMT)),'999,999,999') AS 매출합계
FROM view_iolist
